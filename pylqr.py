...
class PyLQR_iLQRSolver:
    ...
    def __init__(self, T, plant_dyn, cost, verbose=True,constraints=None):
        """
        T:              Length of horizon
	...
	CovQuu = [None] * self.T
	    ...
	    #CovQuu[t] = self.nearPSD(-inv_Quu) #approximate to nearest PSD
            CovQuu[t] = self.nearPSD(inv_Quu)
	    #CovQuu[t] = self.nearPD(inv_Quu)

	    l, _ = np.linalg.eig(CovQuu[t])
    ...
    # the part of the code that returns the nearest p.s.d matrix comes from https://www.stackoverflow.com/questions/10939213/how-can-i-calculate-the-nearest-positive-semi-definite-matrix sega_sai's answer
	...
	s[ s<0 ] = 0.0
	output = np.dot(u, np.dot( np.diag(s), v))
	return output
 
    def nearInvPSD(self, mat):
        u, s, v = np.linalg.svd(mat)
